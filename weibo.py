...
class Weibo(object):
    def __init__(self, config):
        ...
        # 起始时间，即爬取发布日期从该值到现在的微博，形式为 yyyy-mm-dd
        ...
        if "mysql" in self.write_mode:
            self.mysql = MySQL(self.mysql_config)

    def validate_config(self, config):
        argument_lsit = ["filter", "original_pic_download", "retweet_pic_download", "original_video_download",
                         "retweet_video_download"]
        for argument in argument_lsit:
            if config[argument] != 0 and config[argument] != 1:
                sys.exit("%s should be 0 or 1, please check".format(config[argument]))

        # verify since_date
        since_date = str(config['since_date'])
        if (not self.is_date(since_date)) and (not since_date.isdigit()):
            sys.exit("since_date should be yyyy-mm-dd or digit, please check")

        # verify write_mode
        write_mode = ["csv", "mongo", "mysql"]
        if not isinstance(config["write_mode"], list):
            sys.exit("write_mode should be a list")
        for mode in config['write_mode']:
            if mode not in write_mode:
                sys.exit("%s is invalid, csv, mongo, mysql is valid value".format(mode))

        # verify user_id_list
        user_id_list = config['user_id_list']
        if (not isinstance(user_id_list, list)) and (not user_id_list.endswith('.txt')):
            sys.exit(u'user_id_list should be a list or path to txt')
        if not isinstance(user_id_list, list):
            if not os.path.isabs(user_id_list):
                user_id_list = os.path.split(os.path.realpath(__file__))[0] + os.sep + user_id_list
            if not os.path.isfile(user_id_list):
                sys.exit(u'no such file: %s' % user_id_list)
    ...
    def user_to_database(self):
        if 'mysql' in self.write_mode:
            self.mysql.insert('user', [self.user])
    ...
    def get_long_weibo(self, id):
        url = "https://m.weibo.cn/detail/%s".format(id)
                ...
                if w.get(key):
                    ...
                    if file_type == 'img' and ',' in w[key]:
                        ...
                        for j, url in enumerate(w[key]):
                            ...
                            file_name = file_prefix + '_' + str(j + 1) + file_suffix
                        ...
                        file_path = file_dir + os.sep + file_name
                        self.download_one_file(w[key], file_path, file_type, w['id'])
    ...
    def parse_weibo(self, weibo_info):
        ...
        weibo['attitudes_count'] = self.string_to_int(weibo_info['attitudes_count'])
        weibo['comments_count'] = self.string_to_int(weibo_info['comments_count'])
        weibo['reposts_count'] = self.string_to_int(weibo_info['reposts_count'])
    ...
    def get_filepath(self, type):
        """获取结果文件路径"""
        try:
            file_dir = os.path.split(os.path.realpath(__file__))[0] + os.sep + 'weibo' + os.sep + self.user[
                'screen_name']
    ...
    def weibo_to_mysql(self, wrote_count):
        ...
        for w in self.weibo[wrote_count:]:
            ...
            weibo_list.append(w)
        # 在'weibo'表中插入或更新微博数据
        self.mysql.insert('weibo', retweet_list)
        self.mysql.insert('weibo', weibo_list)
    ...
    def get_user_list(self, file_name):
        ...
        with open(file_name, 'rb') as f:
            ...
            user_id_list = [line.split(' ')[0] for line in lines if
...
if __name__ == '__main__':
    try:
        pwd = os.path.split(os.path.realpath(__file__))[0] + os.sep
        config_path = pwd + 'config.json'
        if not os.path.isfile(config_path):
            sys.exit(u'当前路径：%s 不存在配置文件 config.json' % (pwd))
        with open(config_path) as f:
            config = json.loads(f.read())
            weibo = Weibo(config)
            weibo.start()
    except ValueError:
        print(u'config.json 格式不正确，请参考 https://github.com/dataabc/weibo-crawler#3程序设置')
    except Exception as e:
        print('Error: ', e)
        traceback.print_exc()
