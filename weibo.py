...
class Weibo(object):
  def __init__(self, config):
    ...
    def validate_config(self, config):
      argument_lsit = ["filter", "original_pic_download", "retweet_pic_download", "original_video_download", "retweet_video_download"]
    ...
    def get_filepath(self, type):
      """获取结果文件路径"""
      try:
        file_dir = os.path.split(os.path.realpath(__file__))[0] + os.sep + 'weibo' + os.sep + self.user['screen_name']
    ...
    def get_user_list(self, file_name):
      ...
      with open(file_name, 'rb') as f:
        ...
        user_id_list = [line.split(' ')[0] for line in lines if len(line.split(' ')) > 0 and line.split(' ')[0].isdigit()]
      return user_id_list
